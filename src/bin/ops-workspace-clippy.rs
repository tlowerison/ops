...
fn main() -> Result<(), Error> {
    ...
    for git_status in git_statuses {
        ...
        match git_status {
            GitStatus::Added { file }
            | GitStatus::FileTypeChanged { file }
            | GitStatus::Modified { file } => {
    ...
    let workspace_dependencies = workspace_cargo
        ...
        .get("dependencies")
        .ok_or_else(|| {
            Error::msg("cannot parse workspace Cargo.toml: missing key `workspace.dependencies`")
        })?
        .as_table()
        .ok_or_else(|| {
            Error::msg(
                "cannot parse workspace Cargo.toml: key `workspace.dependencies` must be a table",
            )
        })?;
    ...
    let mut package_cargos = HashMap::<String, Value>::from_iter(
        package_paths
            ...
            .map(|package_path| {
                let package_cargo =
                    fs::read_to_string(package_path.join("Cargo.toml"))?.parse::<Value>()?;
    ...
    let changed_package_names = package_cargos
        .keys()
        .map(String::from)
        .collect::<HashSet<_>>();
    ...
    if verbose {
        if top_level_changed_package_names.is_empty() {
            ...
            println!(
                "{}",
                "found changes in these packages (and possibly in their internal dependencies):"
                    .dimmed()
            );
            for package_name in top_level_changed_package_names.iter() {
                ...
                println!(
                    "{}",
                    format!(" - {package_name} ({})", package_path.display()).dimmed()
                );
            }
            println!();
        }
...
fn get_cargo_package_of_file(
    ...
    while let Some(parent) = cur_path.parent() {
        ...
        if let Some(package_path) = package_paths
            .get(parent)
            .map(|package_path| package_path.to_path_buf())
        {
